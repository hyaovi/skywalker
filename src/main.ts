...
engine.subscribeOnce("engine-started", async () => {
  ...
  const radius = 12;
  const satelite = await engine.manager.createEntityWithParams('primitive', { type: 'sphere', material: 'normal', color: '' });
  ...
  const galaxy = engine.manager.createEntity(createGalaxy(6000, radius, 0xccccff));
  engine.manager.activateEntity(galaxy)
  engine.subscribe('engine-update', () => {
    galaxy.sceneObject.rotation.y += 0.0005


    satelite.sceneObject.position.x = radius * 1.01 * Math.sin(performance.now() / 100);
    satelite.sceneObject.position.y = radius * 1.01 * Math.cos(performance.now() / 100);
